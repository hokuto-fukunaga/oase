...
configure_django(){
    ...
    if [ $pip_list -lt 11 ]; then
        if [ "${oase_os}" == "RHEL8" ]; then
            pip3 install requests ldap3 pycrypto openpyxl==2.5.14 xlrd configparser fasteners djangorestframework  django-simple-history pyyaml >> "$OASE_INSTALL_LOG_FILE" 2>&1
        else
            pip install requests ldap3 pycrypto openpyxl==2.5.14 xlrd configparser fasteners djangorestframework django-simple-history pyyaml >> "$OASE_INSTALL_LOG_FILE" 2>&1
        fi
        if [ $? -ne 0 ]; then
            log "ERROR:Installation failed requests ldap3 pycrypto openpyxl==2.5.14 xlrd configparser fasteners djangorestframework django-simple-history pyyaml"
            func_exit
        fi
    else
